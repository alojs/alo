...
export const hydrateObserver = function <H extends (props: any) => any>(
  ...
  if (wrapObserver) {
    if (
      ...
      ChildCompontent.prototype instanceof Component
    ) {
  ...
  const MemoComp = memo(function (props: any) {
...
(async () => {
  await writeFile(
    pkgDir("react", "lib", "observer.tsx"),
    observerHeader() + observerHeaderReact() + observerBody()
  );
  await writeFile(
    pkgDir("preact", "lib", "observer.tsx"),
    observerHeader() + observerHeaderPreact() + observerBody()
  );
})();
