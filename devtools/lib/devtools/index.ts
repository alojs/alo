...
import { Store, actionTypes, dispatchBatch } from "alo/store";
...
export type GlobalDevtoolsState = {
  ...
  timemachines: Observable<{ [index: string]: Timemachine }>;
};
...
export class Devtools extends Observer {
  ...
  "payload": ""
    ...
    if (parentEl) {
      setChildren(parentEl, [...toArray(parentEl.childNodes), this]);
      this.observe(() => {
