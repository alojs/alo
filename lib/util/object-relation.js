...
ObjectRelation.prototype.isRelatedToFunction = function (config, relationObj) {
  var result = false

  if (u.isString(relationObj) || u.isInteger(relationObj)) {
    result = this[config.relationByIdPropertyName][relationObj] !== undefined
  } else if (config.isRelationObject(relationObj)) {
    result = this[config.relationByIdPropertyName][relationObj.getId()] !== undefined
  } else {
