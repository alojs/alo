...
export const removeProp = function <T extends {}, K extends keyof T>(
  obj: T,
  key: K
) {
  const { propObserverIdSetMap, propGetterMap } = observableInfoMap[
    (obj as any).__observableId
  ];
  delete obj[key];
 ...
 */
export const getProp = function <T extends {}, K extends keyof T>(
  obj: T,
  key: K
): T[K] {
  const { propGetterMap } = observableInfoMap[(obj as any).__observableId];
...
const observableArray = function <T extends Observable, K extends keyof T>(
  value: any[],
  key: K,
  parentObj: T
) {
...
export const setProp = function <T extends {} | any[], K extends keyof T>(
  obj: T,
  key: K,
  value: T[K]
) {
...
export function observable<T extends Record<string, any>>(
  ...
  for (const key of Object.keys(obj)) {
    setProp(obj as any, key, obj[key]);
  }
...
export function notify<T extends Observable, K extends keyof T>(
  obj: T,
  key: K
) {
