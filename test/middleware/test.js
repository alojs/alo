...
test('Middleware meddle should call the customizers', function () {
...
test('Middleware meddle should provide the store to the customizers', function () {
  dispatchResult.then(function () {
    assert.equal(true, middleware3store)
  })
})

test('Middleware meddle should call the related middlewares', function () {
  dispatchResult.then(function () {
...
test('Middleware meddle should meddle by taking the dispatch arguments and throwing them through the chained middlewares', function () {
  dispatchResult.then(function () {
