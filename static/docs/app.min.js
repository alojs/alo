...
function mount (parent, child, before) {
  ...
  if (child === childEl && childEl.__@lufrai/redom_view) {
    // try to look up the view if not provided
    child = childEl.__@lufrai/redom_view;
  }

  if (child !== childEl) {
    childEl.__@lufrai/redom_view = child;
  }

  var wasMounted = childEl.__@lufrai/redom_mounted;
...
function doMount (child, childEl, parentEl, oldParent) {
  var hooks = childEl.__@lufrai/redom_lifecycle || (childEl.__@lufrai/redom_lifecycle = {});
  ...
  if (!hooksFound) {
    childEl.__@lufrai/redom_mounted = true;
    return;
  }
  ...
  if (remount || (!triggered && (traverse && traverse.__@lufrai/redom_mounted))) {
  ...
  while (traverse) {
    ...
    var parentHooks = traverse.__@lufrai/redom_lifecycle || (traverse.__@lufrai/redom_lifecycle = {});

    for (var hook in hooks) {
      parentHooks[hook] = (parentHooks[hook] || 0) + hooks[hook];
    }

    if (!triggered && (traverse === document || (parent && parent.__@lufrai/redom_mounted))) {
...
function doUnmount (child, childEl, parentEl) {
  var hooks = childEl.__@lufrai/redom_lifecycle;

  if (!hooks) {
    childEl.__@lufrai/redom_mounted = false;
    return;
  }

  var traverse = parentEl;

  if (childEl.__@lufrai/redom_mounted) {
  ...
  while (traverse) {
    ...
    if (!hooksFound) {
      traverse.__@lufrai/redom_lifecycle = null;
    }
...
function trigger (el, eventName) {
  if (eventName === 'onmount') {
    el.__@lufrai/redom_mounted = true;
  } else if (eventName === 'onunmount') {
    el.__@lufrai/redom_mounted = false;
  }

  var hooks = el.__@lufrai/redom_lifecycle;

  if (!hooks) {
    return;
  }

  var view = el.__@lufrai/redom_view;
  var hookCount = 0;
...
function List (parent, View, key, initData) {
  this.__@lufrai/redom_list = true;
  this.View = View;
